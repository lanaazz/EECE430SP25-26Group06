...
class PlayerMatchStatsForm(forms.ModelForm):
    """Form for recording per-player match statistics (SCRUM-15)."""
    class Meta:
        model = PlayerMatchStats
        fields = ['strikes', 'blocks', 'service_aces', 'errors', 'digs', 'assists']
        widgets = {f: forms.NumberInput(attrs={'class': 'form-input', 'min': 0}) for f in ['strikes', 'blocks', 'service_aces', 'errors', 'digs', 'assists']}


class AttendanceForm(forms.ModelForm):
    """Form for recording session attendance (SCRUM-17)."""
    class Meta:
